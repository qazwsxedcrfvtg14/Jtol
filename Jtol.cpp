...
namespace Jtol{
    ...
    void Alert(string content,string titile){
        MessageBox( 0, content.c_str(), titile.c_str(), MB_SETFOREGROUND );
        }
    ...
    void SetTransparenty(HWND hWnd,int alpah){
        SetWindowLong(hWnd, GWL_EXSTYLE, GetWindowLong(hWnd, GWL_EXSTYLE) | WS_EX_LAYERED);
        SetLayeredWindowAttributes(hWnd, 0, 255, LWA_ALPHA);
        }
    void SetTransparentyColor(HWND hWnd,int color,int alpah){
        SetWindowLong(hWnd, GWL_EXSTYLE, GetWindowLong(hWnd, GWL_EXSTYLE) | WS_EX_LAYERED);
